...
/**
 * Generate SQL for importing all menu items
 */
...
class ItemSQLGenerator {
  ...
  async generate(csvPath) {
    ...
    // Write to file
    fs.writeFileSync('items-import.sql', sql);
    console.log('✅ SQL file generated: items-import.sql');
    console.log(`📊 Generated SQL for ${this.items.length} items`);
  }
}
...
async function main() {
  ...
  if (args.length < 1) {
    console.log(`
Usage: node generate-items-sql.js <csv-file>

Example:
  node generate-items-sql.js "new Menu csv.csv"
    `);
    process.exit(1);
  }
  ...
  const generator = new ItemSQLGenerator();
  await generator.generate(csvPath);
}
...
module.exports = { ItemSQLGenerator };
